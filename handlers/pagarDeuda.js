...
const { toUSD, fmt, calculateNetBalance, updateAccountBalance, netBalanceText } = require('../utils/helpers');

const CANCEL_BTN = Markup.button.callback('❌ Cancelar', 'pd:x');

async function startPagarDeuda(ctx) {
  clearSession(ctx.chat.id);
  const bal = await calculateNetBalance();
  const balText = netBalanceText(bal);

  setSession(ctx.chat.id, { command: 'pagarDeuda', step: 'enterAmount', data: {} });
  await ctx.reply(
...
async function askFromAccount(ctx, payer) {
  ...
  const buttons = accounts.map(a => [Markup.button.callback(`${a.name} ($${fmt(a.balance)})`, `pd:frm:${a._id}`)]);
...
async function askToAccount(ctx, receiver) {
  ...
  const buttons = accounts.map(a => [Markup.button.callback(`${a.name} ($${fmt(a.balance)})`, `pd:to:${a._id}`)]);
  buttons.push([CANCEL_BTN]);
...
async function askNote(ctx) {
  ...
  await ctx.reply('📝 *¿Nota?*', {
    parse_mode: 'Markdown',
    ...Markup.inlineKeyboard([[Markup.button.callback('⏭️ Omitir', 'pd:nte:sk'), CANCEL_BTN]]),
  });
}
...
async function askConfirm(ctx) {
  ...
  const bal = await calculateNetBalance();
  const newTotal = bal.saldoTotal - data.amountUSD; // rough estimate
  ...
  const lines = [
    ...
    `📤 *Desde:* ${from.name}`,
    `📥 *Hacia:* ${to.name}`,
    `📊 *Saldo estimado después:* $${fmt(Math.abs(newTotal))} USD`,
  ];
  if (data.note) lines.push(`📝 *Nota:* ${data.note}`);
  updateSession(ctx.chat.id, { step: 'confirm' });
  await ctx.reply(
    `*Resumen del pago:*\n\n${lines.join('\n')}`,
    { parse_mode: 'Markdown', ...Markup.inlineKeyboard([[Markup.button.callback('✅ Confirmar', 'pd:ok'), CANCEL_BTN]]) }
  );
}

function register(bot) {
  bot.command('pagardeuda', startPagarDeuda);
  bot.action(/^mn:pd$/, async (ctx) => { await ctx.answerCbQuery(); await startPagarDeuda(ctx); });
  ...
  bot.action(/^pd:/, async (ctx) => {
    ...
    if (!session || session.command !== 'pagarDeuda') return;
    ...
    if (action === 'ok') {
      ...
      clearSession(ctx.chat.id);
      await ctx.reply('✅ *Pago de deuda registrado.*', { parse_mode: 'Markdown' });
    }
  });
}

async function determinePayer(ctx) {
  const bal = await calculateNetBalance();
  const s = getSession(ctx.chat.id);
  // If saldoTotal > 0 → Nohelia owes Antonio → Nohelia pays
  const payer = bal.saldoTotal >= 0 ? ctx.users.nohelia : ctx.users.antonio;
  const receiver = bal.saldoTotal >= 0 ? ctx.users.antonio : ctx.users.nohelia;
  updateSession(ctx.chat.id, {
    data: { ...s.data, payerId: payer._id, receiverId: receiver._id },
  });
  ...
  if (session.step === 'enterAmount') {
    ...
    await ctx.reply('💱 *¿En qué moneda?*', {
      ...
      ...Markup.inlineKeyboard([
        [Markup.button.callback('🇺🇸 USD', 'pd:cur:usd'), Markup.button.callback('🔢 Otra', 'pd:cur:oth')],
        [CANCEL_BTN],
      ]),
    });
    return;
